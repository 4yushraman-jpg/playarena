...
func uploadImageWithPrimary(t testing.TB, ts *testServer, path, entityType, entityID, token string) *http.Response {
	t.Helper()
	var buf bytes.Buffer
	w := multipart.NewWriter(&buf)
...
func TestMedia_PrimarySwap(t *testing.T) {
	...
	upA := uploadImageWithPrimary(t, ts, mediaURL(actor.orgSlug), "organization", actor.orgID, actor.token)
	...
	upB := uploadImageWithPrimary(t, ts, mediaURL(actor.orgSlug), "organization", actor.orgID, actor.token)
