...
import { PlusIcon, ShieldIcon, SearchIcon, XIcon } from "lucide-react"
...
function buildColumns(orgSlug: string): ColumnDef<Team, unknown>[] {
  return [
    {
      id: "team",
      header: "Team",
    ...
    },
    {
      accessorKey: "status",
      header: "Status",
      cell: ({ getValue }) => <StatusBadge status={getValue() as TeamStatus} />,
    },
    {
...
export default function TeamsPage() {
  ...
  const statusFilter = (searchParams.get("status") ?? "all") as TeamStatus | "all"
  ...
  const { data, isLoading, isError, refetch } = useTeamList(orgSlug, queryParams)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Dashboard", href: `/${orgSlug}` }, { label: "Teams" }]}
        action={
          canCreate ? (
      ...
      {isError ? (
        ...
        <EmptyState
          icon={<ShieldIcon />}
          title="No teams yet"
          description="Create your first team to start managing rosters and assigning players."
          action={
            canCreate ? (
              <Button asChild size="sm" className="gap-1.5">
                <Link href={`/${orgSlug}/teams/new`}>
