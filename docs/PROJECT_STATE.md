...
**Build status:** backend `go build ./...` / `go vet ./...` / `sqlc generate` clean; frontend `tsc --noEmit` 0 errors, `eslint` 0 warnings, `vitest` **325/325** passing, `next build` clean  
...
**Frontend phases complete:** FE-1, FE-2, FE-3, FE-4, FE-5, FE-6, **FE-7** (FE-7A + FE-7BA + FE-7BB + FE-7BC — Matches & Live Scoring), **FE-8A** (Walkover Support — end-to-end no-show handling + standings integration), **FE-8B** (Bracket Progression — next-match linkage + atomic winner propagation), **FE-8C** (Tournament Automation & Fixture Generation — deterministic engine + generation wizard)  
**Platform status:** A real Kabaddi tournament can be **organized** end-to-end through PlayArena — create tournament → approve registrations → **generate the full schedule with a wizard (no manual fixtures)** → start → score live (offline-tolerant) → complete with winner → bracket winner auto-advances → standings refresh → complete tournament — without database access, manual score correction, or developer intervention. No-shows are handled via walkover (FE-8A); the `is_walkover` flag is preserved for future Reputation-Point exclusion. Brackets are wired and self-advance (FE-8B). Fixture generation supports round robin, league, knockout, and group + knockout with deterministic/reproducible seeding (FE-8C).
...
#### FE-8C — Tournament Automation & Fixture Generation

**Status: COMPLETE.** Final core slice of the FE-8 blueprint — eliminates manual fixture entry. A director can now generate and run a tournament without constructing a single bracket by hand. Double-elimination, GP-2, rankings, recruitment, multi-sport, and AI scheduling are explicitly out of scope. Adversarially reviewed; all P0/P1 resolved. **No new migration** (reuses FE-8B's relaxed participant CHECK + bracket columns). Full report: `docs/architecture/fe8c-fixture-generation.md`.

- **Fixture Generation ✓** — Pure, deterministic engine `internal/fixtures` (no DB, no I/O, no global state) producing a fixture graph + bracket graph + progression graph from (participants, format, seed strategy, group config). Thin generation service `internal/fixturegen` persists a plan atomically. Endpoints: `POST .../tournaments/{id}/fixtures/generate` (dry-run preview **and** confirm), `POST .../fixtures/resolve-qualifiers`, `GET .../fixtures/generation` (audit record). All gated on `tournament.update`.
- **Round Robin ✓** — Circle method; every pair exactly once; rotating bye for odd counts; home/away balanced to ±1 via an Eulerian orientation of K_n. Proven: no duplicate / missing / self fixtures.
- **League ✓** — Double round-robin (legs=2): every pair home and away → perfect home/away balance.
- **Knockout ✓** — Seeded single-elimination for any size (8/16/32/64 clean; non-powers padded with byes assigned to top seeds for seed protection; byes are direct round-2 placement, never phantom matches). Top seeds meet latest (standard seed-slot order). Every generated knockout is fully wired with FE-8B `next_match_id`/`next_match_slot`, so winners auto-advance with zero new progression code.
- **Group + Knockout ✓** — Snake/serpentine group distribution; per-group round-robin; knockout seeded RANK-MAJOR from group qualifiers so round 1 pairs winner-vs-runner-up of **different** groups (no same-group immediate rematch for even group counts) with a group's own qualifiers in opposite halves. Qualifier slots carry a `{group,rank}` mapping in `matches.metadata`; **resolve-qualifiers** computes per-group standings once the group stage concludes and fills the knockout's first round (idempotent, all-or-nothing), after which FE-8B propagation takes over.
- **Deterministic Seeding ✓** — Manual (seed_number), registration-order, and random strategies, all on total orders (id is the final tiebreak; no map-iteration dependence). Random is a fixed-seed Fisher–Yates: the server mints the seed on a random preview, returns it, and re-applies it on confirm → the persisted bracket is byte-identical to the preview. Seed masked to 52 bits to round-trip exactly through a JSON number. **Same seed ⇒ identical bracket** (proven).
- **Bracket Automation ✓** — Generated matches are ordinary rows, so FE-8A walkover and FE-8B atomic winner propagation apply unchanged; a generated knockout self-advances end-to-end. Safety: generation only runs from `registration_closed` with **no existing fixtures**, under a `FOR UPDATE` lock with in-transaction re-validation — double / partial / re-generation and races are impossible (all-or-nothing). A `expected_participants` guard rejects a stale preview after a late registration.
- **Tournament Generation Wizard ✓** — 5-step director workflow (Format → Settings → Review structure → Preview fixtures → Confirm). Nothing is created until the final confirm; the preview is the exact persisted structure (random seed included). Plus an explainability/audit panel (Generated by / Format / Seed strategy / Random seed / Generated at — reproducible) and a one-click "Resolve qualifiers" action that self-reveals only when a group stage is complete.

**Adversarial review (focus: seeding, byes, group advancement, progression, duplicate/re-generation, walkover interaction, late registration, stale state):** all P0 (double/concurrent generation, partial generation, broken/orphan progression links, self/cross-tournament links) and P1 (no dup/missing/self fixtures, bye seed-protection, no same-group round-1, reproducible random, late-registration guard, reg-closed-only window, deterministic group advancement, walkover interaction) resolved. Residual **P2** (documented, non-blocking): group ranking uses standard 3/1/0 points; odd-group / non-power-of-two qualifier fields emit warnings rather than being prevented; re-preview required after late registration (intentional); no graphical bracket tree (sectioned list); resolution is a manual one-click action (intentional).

**Tests after FE-8C: 325 frontend across 36 files** (+7: slot resolver, full wizard flow with seed echo + expected_participants, preview-error handling, panel states). Backend: pure-engine suite (RR all-pairs/no-dup/no-self, ±1 balance, knockout 8/16/32/64 progression-integrity invariant, byes, group no-same-group-R1, random determinism, validation) + `fixturegen/integration` (dry-run-vs-confirm, knockout wiring, safety guards, late-registration, explainability/reproducible random, group+knockout resolve end-to-end). `go build`/`go vet`/`gofmt` clean. **Scope stopped at FE-8C — FE-8D (double-elimination) not started.**

> **Validation note:** running the *entire* backend `go test ./...` at once aborts on this Windows dev box with testcontainers' "rootless Docker is not supported" under parallel container load (it also breaks the unrelated `members/integration` suite). The FE-8C suites pass when run in isolation; CI (Linux) runs them cleanly.

**Design decisions / lessons (FE-8C):**
- **The seed order is the single root of all structure**, so generation is explainable by construction — store and return it, and every placement is justifiable.
- **Reproducible random needs the seed to round-trip through JSON**: a naive 63-bit seed loses precision in JavaScript and silently breaks "same seed ⇒ same bracket"; mask to 52 bits.
- **Byes are placement, not matches** — a one-sided "match" violates the participant CHECK; place the bye recipient directly into round 2 (relies on FE-8B's relaxed CHECK for the resulting partial round-2 match).
- **Group→knockout resolution is a separate, deterministic step** (results are unknown at generation): generate qualifier *slots* with a `{group,rank}` mapping, resolve from standings later. Keeps the engine pure.
- **Build the plan inside the generation transaction** from the locked participant snapshot, so the persisted bracket always matches the participants that actually existed at commit.

---

## 11. Next Strategic Review Required

**Backend phases 1 – 23D, GP-1, and frontend phases FE-1 – FE-7 plus FE-8A, FE-8B, and FE-8C are complete.** PlayArena has crossed from "can run tournaments" to "can organize tournaments": create → approve → **generate (wizard, no manual fixtures)** → start → live score → complete (incl. walkover) → bracket auto-advances → [group+knockout: resolve qualifiers] → standings → complete tournament, production-validated (325 frontend tests; backend suites green). The FE-8 core is delivered; the remaining FE-8 item is FE-8D (double-elimination), which is optional.

This is a natural decision point. **Roadmap priorities must be re-evaluated in a dedicated strategic review before starting GP-2 or any subsequent initiative.** GP-2 is *not* automatically next. The review should weigh the open directions in §8 (player persona activation, reputation/ranking system, recruitment, public profiles, FE-8D (optional) — double-elimination, production hardening, news module, multi-instance SSE) against current product goals, and select the highest-value next move explicitly.
